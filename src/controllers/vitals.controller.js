...
export const ingestVitals = async (req, res) => {
  try {
    ...
    if (isDbConnected) {
      ...
      console.warn('⚠️ MongoDB not connected — running in fallback mode (no persistence)');
      // create transient objects to allow alert flow to continue for testing
      patient = {
        patientId,
        name: patientId,
        // use TWILIO_WHATSAPP_TO (strip whatsapp: prefix) as a fallback recipient
        phone: (process.env.TWILIO_WHATSAPP_TO || '').replace(/^whatsapp:/, ''),
      };
      vitals = { patientId, deviceId, heartRate, spo2, temperature };
    }
