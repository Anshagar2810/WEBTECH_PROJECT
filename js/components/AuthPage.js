        ...
        if (userType === 'admin') {
            if (isLogin) {
                ...
                if (formData.email && formData.password) {
                    if (adminAccount) {
                        // Check against stored admin account
                        if (formData.email === adminAccount.email && formData.password === adminAccount.password) {
                            onLogin({ name: adminAccount.name, email: adminAccount.email }, 'admin');
                        } else {
                ...
                if (setAdminAccount) {
                    ...
                    onLogin({ name: newAdmin.name, email: newAdmin.email }, 'admin');
                } else {
                    console.error("setAdminAccount is missing!");
                    // Fallback
                    onLogin({ name: formData.name, email: formData.email }, 'admin');
                }
            }
            ...
            if (isLogin) {
                ...
                const doctor = doctors.find(d => {
                    ...
                    const matchEmail = d.email && d.email.toLowerCase() === input.toLowerCase();
                    const matchId = d.id && d.id.toLowerCase() === input.toLowerCase();
                    const matchPhone = d.phone && d.phone === input;
                    
                    return (matchEmail || matchId || matchPhone) && d.password === formData.password;
                });
                
                if (doctor) {
                    onLogin(doctor, 'doctor');
                } else {
                    alert('Invalid Doctor credentials! Please check your email/ID and password.');
                }
            } else {
                // Doctor Signup
                if (formData.password !== formData.confirmPassword) {
             ...
             if (isLogin) {
                ...
                const nurse = nurses.find(n => {
                    const input = formData.email.trim();
                    const matchEmail = n.email && n.email.toLowerCase() === input.toLowerCase();
                    const matchId = n.id && n.id.toLowerCase() === input.toLowerCase();
                    
                    return (matchEmail || matchId) && n.password === formData.password;
                });
                
                if (nurse) {
                    onLogin(nurse, 'nurse');
                } else {
                    alert('Invalid Nurse credentials! Please check your email/ID and password.');
                }
            } else {
                // Nurse Signup
                if (formData.password !== formData.confirmPassword) {
                ...
                const newNurse = {
                    id: `NURSE-${Date.now().toString().slice(-6)}`,
                    name: formData.name,
                    email: formData.email,
                    phone: formData.phone,
                    password: formData.password,
                        ...
                        <h2 className="text-xl font-bold text-gray-800">
                            {isForgotPassword 
                                ...
                                : (userType === 'admin' 
                                    ...
                                    : (isLogin ? (userType === 'nurse' ? 'Nurse Login' : 'Doctor Login') : 'Staff Registration'))}
                        </h2>
